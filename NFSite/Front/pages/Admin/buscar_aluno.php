...
<div class="container">
    <h1>Convocação de Alunos</h1>

    <!-- Barra de Pesquisa -->
    <div class="search-container">
        <input type="text" id="search" placeholder="Pesquisar Alunos..." onkeyup="filterAlunos()">
    </div>
    ...
    <div id="alunos-container">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<div class="aluno-card" data-id="' . $row['id'] . '" data-nome="' . $nome . '">';
    ...
    <!-- Botão de Convocar Professor -->
    <button class="btn-convocar-professor" onclick="location.href='buscar_professor.php'">Convocar Professor</button>

</div>

<script>
    // Função para filtrar os alunos na busca
    function filterAlunos() {
        ...
        alunos.forEach(aluno => {
            ...
            if (nome.includes(search)) {
                aluno.style.display = 'inline-block';
            } else {
    ...
    // Marcar o aluno como selecionado ao clicar
    const alunos = document.querySelectorAll('.aluno-card');
    alunos.forEach(aluno => {
        aluno.addEventListener('click', () => {
            aluno.classList.toggle('selected');
        });
    });

    // Função para salvar a convocação
    function saveConvocacao() {
        const selecionados = document.querySelectorAll('.aluno-card.selected');
        const idsSelecionados = Array.from(selecionados).map(card => card.dataset.id);
        
        if (idsSelecionados.length > 0) {
            // Enviar os dados dos alunos selecionados para salvar a convocação
            alert('Convocação salva para os alunos com ID: ' + idsSelecionados.join(', '));
