...
$professor_id = $_SESSION['professor_id'] ?? 1; // Ajuste conforme sessão real
...
$t_result = $conn->query("SELECT id, nome FROM turmas");
...
tbody td:last-child {
  ...
  gap: 12px;
  margin-top: 10px;
  flex-wrap: nowrap;   /* Força os botões numa única linha */
      ...
      <tbody>
        <?php while ($t = $treinos->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              <a href="editar_treino.php?id=<?= $t['id'] ?>" class="btn btn-edit" aria-label="Editar treino do dia <?= date('d/m/Y', strtotime($t['data'])) ?>"><i class="bi bi-pencil-square"></i></a>
              <a href="?excluir=<?= $t['id'] ?>" class="btn btn-delete" onclick="return confirm('Tem certeza que deseja excluir este treino?')" aria-label="Excluir treino do dia <?= date('d/m/Y', strtotime($t['data'])) ?>"><i class="bi bi-trash"></i></a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
